...
class SEDTask4(pl.LightningModule):
    ...
    def training_step(self, batch, batch_indx):
        ...
        # batch: audio, atst_feats, labels, padded_indxs, filenames
    ...
    def _align_scores_with_ground_truth(self, scores_dict, ground_truth, audio_durations):
        ...
        for sid, sdf in scores_dict.items():
            ...
            if sid in gt_ids:
                ...
                # keep as-is; may be filtered out later
                target_id = sid
    ...
    def on_validation_epoch_end(self):
        ...
        real_teacher_strong_loss = sum(self.val_loss_real_teacher) / len(self.val_loss_real_teacher) if len(self.val_loss_real_teacher) > 0 else 0.0
    ...
    def on_test_epoch_end(self):
        ...
        if self.evaluation:
            ...
            if self.fast_dev_run:
                ...
                # Align test scores with ground truth to handle missing files/ID mismatches
                scores_student, gt_aligned, dur_aligned = self._align_scores_with_ground_truth(
                    self.test_scores_postprocessed_buffer_student, ground_truth, audio_durations
                )
                scores_teacher, gt_aligned_t, dur_aligned_t = self._align_scores_with_ground_truth(
                    self.test_scores_postprocessed_buffer_teacher, ground_truth, audio_durations
                )
                ground_truth = gt_aligned
                audio_durations = dur_aligned
